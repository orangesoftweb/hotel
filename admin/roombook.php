							...
							<div class="panel-footer">
								<form method="post">
									...
									<select name="conf"class="form-control">
										...
										<option value="accepted">Aceptar Reserva</option>
										<option value="denied">Rechazar Reserva</option>
									</select>
									</div>
									<input type="submit" name="co" value="Conform" class="btn btn-success">
									<a href="mailer.php?id=<?php echo $id ?>&addressee=<?php echo $title.' '.$fname.' '.$lname ?>&email=<?php echo $email ?>&status=1" class="btn btn-primary">Accept</a>
									<a href="mailer.php?id=<?php echo $id ?>&addressee=<?php echo $title.' '.$fname.' '.$lname ?>&email=<?php echo $email ?>&status=2" class="btn btn-danger">Deny</a>
								</form>
							</div>
						</div>
					</div>
...
{	
	...
	/*if($st=="accepted") {
		...
		$message =  "$title $fname $lname su reservación ha sido aceptada";
		$send = mail($email, $subject, $message);

		if($send) {
			echo "<script type='text/javascript'> alert('Correo enviado exitosamente!')</script>";
		}else {
	...
	extract ($_GET);
	if (htmlspecialchars(!isset($_GET['msj'])))
	$msj=0;
	if ($msj==1)
	echo "<script type='text/javascript'> alert('Correo enviado exitosamente!')</script>";
	if ($msj==2)
